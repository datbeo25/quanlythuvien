...
namespace Login
{
    ...
    public partial class Thehoivien : Form
    {
        ...
        private string connectionString = "Data Source=Acernitro5;Initial Catalog=qlthuvien2;Integrated Security=True"; // Thay YOUR_CONNECTION_STRING bằng chuỗi kết nối của bạn
        ...
        private void LoadData()
        {
            ...
            string query = "SELECT Thedocgia.Mathedocgia, Docgia.Tendocgia, Thedocgia.Ngaycap, Thedocgia.Ngayhethan,Docgia.Madocgia " +
        ...
        private void btn_Caplaithe_Click(object sender, EventArgs e)
        {
            if (selectedMathedocgia != -1)
            {
                ...
                using (SqlCommand command = new SqlCommand(updateThedocgiaQuery, connection))
                {
                    ...
                    connection.Open();
                    int rowsAffected = command.ExecuteNonQuery(); // Số hàng bị ảnh hưởng sau khi cập nhật
                    connection.Close();
            ...
            {
                MessageBox.Show("Vui lòng chọn một thẻ để cập nhật.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
